...
describe('guideNavigation', () => {
  it('turns a direct clearance toward its active fix', () => {
    ...
    aircraft.navigation = { mode: 'direct', fixIds: ['FINAL1'], currentLegIndex: 0, procedure: 'DCT FINAL1' };

    const result = guideNavigation(aircraft, world);

    expect(result.aircraft.targetHeading).not.toBe(aircraft.targetHeading);
    expect(result.aircraft.navigation?.fixIds[0]).toBe('FINAL1');
  });
  ...
  it('enters a hold when it reaches its hold fix', () => {
    ...
    aircraft.navigation = { mode: 'hold', fixIds: ['FINAL1'], currentLegIndex: 0, procedure: 'HOLD FINAL1', holding: false };
